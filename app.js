...
async function saveToFirebase() {
    ...
    try {
        ...
        console.error('Firebase 저장 오류:', e.message);
        return false;
    }
}

// ── 추천번호 Firebase 저장 (records.js에서 호출) ──
window.saveToFirebase = async function(data) {
    ...
    try {
        ...
        console.error('Firebase 추천 저장 오류:', e.message);
        return false;
    }
};
...
window.testSave = async function() {
    var ok = await window.saveToFirebase({
        round: 1106, type: 'basic', numbers: [1,6,7,25,34,42]
    });
    alert(ok ? 'DB 저장 완료!' : '저장 실패 - 콘솔 확인');
};
...
function parseCSV(text) {
    ...
    if (lottoData.length) {
        ...
        saveToLS();
        // Firebase에도 전체 저장
        saveToFirebase().then(function(ok) {
